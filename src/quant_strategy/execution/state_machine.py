...
from quant_strategy.core.models import Position, StrategyState
...
class StrategyStateMachine:
    ...
    def should_force_close(self, now: datetime, minutes_to_close: int) -> tuple[bool, str | None]:
        if not self._state.current_position:
            return False, None
        if minutes_to_close <= self._risk.forced_close_minutes_before_eod:
            return True, "临近收盘触发强平"
        delta = now - self._state.current_position.open_time
        if delta.total_seconds() / 60 >= self._risk.max_position_minutes:
            return True, "持仓超出最大持有时长"
        return False, None
